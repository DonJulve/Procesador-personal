...
static unsigned long multiciclo = 0;
...
void sim(IREG instr)
{
	...
	/* nada que simular en esta etapa */
        ...
        if (etapa_Ain.co == FLOAT){
            multiciclo++;
            if (multiciclo < 5){ //Latencia de 5 ciclos
			    etapa_Aout = inula; //Se impide coger otras instrucciones
			    carga_A = 0; 
			    cfloat++;   //Aumento los ciclos de ejecución de FLOAT
            }
            else{
                //Si ya ha pasado la latencia de 5 ciclos, continua ejecutando la instrucción
                etapa_Aout = etapa_Ain;
                multiciclo = 0;
                carga_A = 1;
		    }
        }
        else { // Si no es instrucción FLOAT continua
            etapa_Aout = etapa_Ain;
            carga_A = 1;
        }
